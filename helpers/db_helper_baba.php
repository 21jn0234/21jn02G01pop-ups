...
{
    $sql = "SELECT * FROM tenpo ORDER BY tenpoName ASC";
...
{
    $sql = "SELECT * FROM syohin 
                    WHERE tenpoCode = :tenpoCode ORDER BY tenpoCode ASC";
...
// ユーザーID(メールアドレス), パスワードが一致する会員データを取得する
function select_puser($dbh,$userId,$pw)
{
    $sql = "SELECT userId,username FROM puser
...
function select_syohin_genre($dbh,$syohinId)
{
    $sql = "SELECT * FROM syohin AS S 
            INNER JOIN genre AS G ON S.genreCode = G.genreCode 
            WHERE syohinId = :syohinId";
...
{
    $sql = "SELECT S.syohinName,S.price,W.wantingId,B.buyId,R.reprintId
    FROM puser AS U
    LEFT OUTER JOIN wanting AS W ON U.userId = W.userId
    LEFT OUTER JOIN syohin AS S ON W.syohinId = S.syohinId 
    LEFT OUTER JOIN bought AS B ON B.syohinId = S.syohinId
	LEFT OUTER JOIN reprint AS R ON R.syohinId = S.syohinId
    WHERE U.userId = :userId AND endDate > CONVERT(DATE,getdate())
    union
    SELECT S.syohinName,S.price,W.wantingId,B.buyId,R.reprintId
    FROM puser AS U
    LEFT OUTER JOIN wanting AS W ON U.userId = W.userId
    LEFT OUTER JOIN syohin AS S ON W.syohinId = S.syohinId 
    LEFT OUTER JOIN bought AS B ON S.syohinId = B.syohinId
	LEFT OUTER JOIN reprint AS R ON R.syohinId = S.syohinId
...
function insert_into_puser($dbh, $userId, $un, $pw)
    {
        ...
        $sql = "INSERT INTO puser(userId, userName, pw) 
                  VALUES (:userId, :un, :pw)";
...
//     return $change;
// }
function wanting_delete($dbh){#欲しリス削除未
